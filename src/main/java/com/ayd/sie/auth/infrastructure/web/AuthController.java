...
public class AuthController {
        ...
        public ResponseEntity<LoginResponseDto> verifyTwoFactor(
                ...
                LoginResponseDto response = authApplicationService.verifyTwoFactor(twoFactorRequest, ipAddress,
                                userAgent);
                return ResponseEntity.ok(response);
        }
        ...
        public ResponseEntity<RefreshTokenResponseDto> refreshToken(
                ...
                RefreshTokenResponseDto response = authApplicationService.refreshToken(refreshRequest, ipAddress,
                                userAgent);
                return ResponseEntity.ok(response);
        }
        ...
        public ResponseEntity<Map<String, Object>> forgotPassword(
                ...
                Map<String, Object> response = new HashMap<>();
                response.put("message", "If the email exists, a password reset code has been sent");
                response.put("timestamp", System.currentTimeMillis());
        ...
        @PostMapping("/request-disable-2fa")
        @Operation(summary = "Request disable 2FA - Step 1", description = "Validate password and send verification code to disable 2FA")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Verification code sent", content = @Content(schema = @Schema(implementation = RequestDisable2FAResponseDto.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid password or 2FA not enabled"),
                        @ApiResponse(responseCode = "401", description = "Authentication required")
        })
        public ResponseEntity<RequestDisable2FAResponseDto> requestDisable2FA(
                        @Valid @RequestBody RequestDisable2FARequestDto request,
                        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails) {

                RequestDisable2FAResponseDto response = authApplicationService.requestDisable2FA(request,
                                userDetails.getUsername());
                return ResponseEntity.ok(response);
        }

        @PostMapping("/confirm-disable-2fa")
        @Operation(summary = "Confirm disable 2FA - Step 2", description = "Verify code and disable two-factor authentication")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "2FA disabled successfully"),
                        @ApiResponse(responseCode = "400", description = "Invalid verification code"),
                        @ApiResponse(responseCode = "401", description = "Authentication required")
        })
        public ResponseEntity<Map<String, Object>> confirmDisable2FA(
                        @Valid @RequestBody ConfirmDisable2FARequestDto request,
                        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails) {

                authApplicationService.confirmDisable2FA(request, userDetails.getUsername());
