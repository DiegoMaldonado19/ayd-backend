...
public interface StateHistoryJpaRepository extends JpaRepository<StateHistory, Integer> {
        ...
        Page<StateHistory> findByBusinessIdOrderByChangedAtDesc(@Param("businessId") Integer businessId,
                        Pageable pageable);

        @Query("SELECT sh FROM StateHistory sh WHERE sh.guide.courier.userId = :courierId ORDER BY sh.changedAt DESC")
        Page<StateHistory> findByCourierIdOrderByChangedAtDesc(@Param("courierId") Integer courierId,
                        Pageable pageable);
        ...
        long countByChangedAtBetween(@Param("startDate") LocalDateTime startDate,
                        @Param("endDate") LocalDateTime endDate);
        ...
        @Query("SELECT sh FROM StateHistory sh WHERE sh.guide.guideId = :guideId ORDER BY sh.changedAt DESC")
        List<StateHistory> findGuideHistory(@Param("guideId") Integer guideId);

        @Query("SELECT sh FROM StateHistory sh WHERE sh.changedAt BETWEEN :startDate AND :endDate")
        List<StateHistory> findByDateRange(@Param("startDate") LocalDateTime startDate,
                        @Param("endDate") LocalDateTime endDate);
}