...
public class BusinessController {
    ...
    public ResponseEntity<GuideResponseDto> createGuide(
        ...
        Integer businessId = getCurrentBusinessId();
        GuideResponseDto response = createTrackingGuideUseCase.execute(createGuideDto, businessId);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Invalid input data or guide cannot be modified"),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
    ...
    public ResponseEntity<GuideResponseDto> updateGuide(
        ...
        Integer businessId = getCurrentBusinessId();
        GuideResponseDto response = updateTrackingGuideUseCase.execute(guideId, updateGuideDto, businessId);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/guides")
    @Operation(summary = "Get business guides", description = "Retrieves all tracking guides for the current business with pagination.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<GuideListResponseDto> getGuides(
            @Parameter(description = "Active guides only") @RequestParam(defaultValue = "false") boolean activeOnly,
            @PageableDefault(size = 20) Pageable pageable) {

        Integer businessId = getCurrentBusinessId();
        GuideListResponseDto response = activeOnly ? getBusinessGuidesUseCase.executeActiveOnly(businessId, pageable)
                : getBusinessGuidesUseCase.execute(businessId, pageable);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<GuideStatusDto> getGuideStatus(
        ...
        Integer businessId = getCurrentBusinessId();
        GuideStatusDto response = getGuideStatusUseCase.execute(guideId, businessId);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<GuideStatusDto> getGuideStatusByNumber(
        ...
        Integer businessId = getCurrentBusinessId();
        GuideStatusDto response = getGuideStatusUseCase.executeByGuideNumber(guideNumber, businessId);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<CancellationResponseDto> cancelGuide(
        ...
        Integer businessId = getCurrentBusinessId();
        CancellationResponseDto response = cancelDeliveryUseCase.execute(guideId, cancelGuideDto, businessId);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<LoyaltyBenefitsDto> getLoyaltyBenefits() {
        Integer businessId = getCurrentBusinessId();
        LoyaltyBenefitsDto response = getLoyaltyBenefitsUseCase.execute(businessId);
        return ResponseEntity.ok(response);
    }

    private Integer getCurrentBusinessId() {
